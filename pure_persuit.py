...
while t <= len(xb) and running:
    ...
    position_text5 = font.render('Time: {}'.format(
        t+1), True, (0, 0, 0), (255, 255, 255))
    ...
    if t > 10:
        ...
        running = False
    if t > 0:
        pygame.draw.line(screen, (0, 255, 0), coord(
            [xf[t-1], yf[t-1]]), coord([xf[t], yf[t]]), 1)
        pygame.draw.line(screen, (255, 0, 0), coord(
            [xb[t-1], yb[t-1]]), coord([xb[t], yb[t]]), 1)
    ...
    if dist <= 10:
        ...
        print('Target caught at Time: {} Bomber position: {} and Fighter position: {}'.format(
            t+1, (xb[t], yb[t]), (xf[t], yf[t])))
        running = False
